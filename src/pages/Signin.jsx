...
export default function Signin(){
    ...
    return(
        ...
        <main className="h-screen z-10 max-w-7xl mx-auto flex md:flex-row relative flex-col">
            ...
            <div className="h-full w-full flex justify-center items-center mt-8 md:mt-0 md:w-1/2 bg-grey-500">
                <div className='flex flex-col gap-y-4 items-center w-full px-8 xl:px-0 xl:w-2/3'>
                    ...
                    <button onClick={handleLogin} className='bg-[#7EBCC3] w-full text-lg py-2 rounded-md text-white mt-2'>Login</button>
                    {error && <p>{error} Something occurred while logging the user</p>}
                    <p className='font-semibold'>Are you new here? <Link to='/signup' className='text-[#7EBCC3]'>Sign up</Link></p>
                </div>
            </div>
        </main>
        </>
